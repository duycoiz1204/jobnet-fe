...
export const HubspotConversationsProvider = ({
  ...
  // AskEmail Stage
  const botDefaultMessage =
    'First of all, enter basis information to save your contact';
  ...
  // ChatflowUI Stage
  const [chatflowUIStageCriteria, setChatflowUIStageCriteria] = useState<{
    isPassed: boolean;
    chatflowChoosen: undefined | 'AI' | 'ASSISTANT';
  }>({
    isPassed: false,
  ...
  const [hubspotUser, setHubspotUser] = useState<{
    aiUser?: ActorHubspotType;
    assistantUser?: ActorHubspotType;
  }>({
    aiUser: undefined,
    assistantUser: undefined,
  });
  ...
  const hideWidget = useCallback(() => {
    setIsOpen(false);
    setChatflowUIStageCriteria({
      ...chatflowUIStageCriteria,
      isPassed: false,
      chatflowChoosen: undefined,
    });
  }, []);
  ...
  useEffect(() => {
    setTimeout(async () => {
      ...
      const contactOfUTK = await hubspotService.getContactByHuspotUtk(
        hubspotutk
      );
      if (contactOfUTK.vid) {
        const vid = contactOfUTK['vid'];
        const identities = contactOfUTK['identity-profiles'][0][
          'identities'
        ] as Array<object>;
        const emailPrimary = identities.find(
          (identity: any) =>
            identity.type == 'EMAIL' && identity['is-primary'] !== undefined
        );
        if (identities.length > 0 && emailPrimary) {
          // Check this one
          ...
          window.HubSpotConversations.widget.load();
          window.HubSpotConversations.widget.refresh();
          setAskEmailStageCriteria({
            ...askEmailStageCriteria,
            isPassedEmail: true,
            botMessage: botChatflowMessage,
            visitorId: vid,
          });
        }
      }
  ...
  const onAskEmailStageSubmit = (
    values: z.infer<typeof AskEmailStageSchema>
  ) => {
  ...
  const onAskEmailStageError = (e: any) => {
    // Catch form clicked
    const { email, name } = e;
    let message = '';
    if (email) {
  ...
  useEffect(() => {
    if (isOpen) {
      (async () => {
        ...
        let isShowChatflow =
          Cookies.get('isShowChatflow') == undefined ||
          Cookies.get('isShowChatflow') == 'true'
            ? true
            : false;
        if (isShowChatflow) {
          isShowChatflow = await hubspotService.isShowChatflowUI({
            visitorId: askEmailStageCriteria.visitorId,
          });
        }
        ...
        if (!isShowChatflow) {
          ...
          const AIUser = await hubspotService.getActor(
            process.env.NEXT_PUBLIC_HUSPOT_AI_ACTOR_ID as string
          );
          const AssistantUser = await hubspotService.getActor(
            process.env.NEXT_PUBLIC_HUSPOT_ASSISTANT_ACTOR_ID as string
          );
          setHubspotUser({
            ...hubspotUser,
            aiUser: AIUser,
  ...
  const handleChatflowChoosen = (chatflow: 'AI' | 'ASSISTANT') => {
    setChatflowUIStageCriteria({
      ...chatflowUIStageCriteria,
      isPassed: true,
      chatflowChoosen: chatflow,
    });
    window.history.pushState(
      {},
      `${chatflow == 'AI' ? 'talk_to_ai' : 'talk_to_assistant'}`,
  ...
  return (
    <HubspotConversationsContext.Provider value={{ isOpen, toggleWidget }}>
      {/* {children} */}
      <div className="fixed bg-transparent max-w-[30%] w-[30%] flex flex-col gap-y-2 duration-500 items-end bottom-5 transition-all right-3">
        <div
          className={`${classes.chatWidgetContainer} ${
            askEmailStageCriteria.isPassedEmail &&
            chatflowUIStageCriteria.isPassed
              ? ''
              : 'hidden'
          }`}
        ...
        />
        <div
          className={`shadow-2xl h-[300px] min-w-[100%] bg-emerald-400 shadow-slate-500/50 rounded-lg flex flex-col gap-y-1 p-3 ${
            isOpen ? '' : 'hidden'
          } ${
            (askEmailStageCriteria.isPassedEmail &&
              chatflowUIStageCriteria.isPassed) ||
        ...
        >
          <div className="h-[80px] grid grid-cols-4  min-w-full">
            {' '}
            ...
            <Image
              ...
              width={60}
              height={60}
              className="justify-self-end self-end animate-bounce animate-duration-2s animate-ease-in-out animate-infinite"
              alt=""
            />
            <div
            ...
            after:border-r-transparent after:-z-10 after:border-b-transparent after:left-[0px] after:bottom-[-10px]"
              dangerouslySetInnerHTML={{
                __html: askEmailStageCriteria.botMessage,
              }}
            ></div>
          </div>
          <div
            className={`${askEmailStageCriteria.isPassedEmail ? 'hidden' : ''}`}
          >
            {' '}
            {/* AskEmail Stage */}
            <Form {...form}>
              <form
                onSubmit={form.handleSubmit(
                  onAskEmailStageSubmit,
                  onAskEmailStageError
                )}
              >
                <FormField
              ...
              <Input id='nameChatWidget' className='min-w-full' placeholder='Mr. John...' /> */}
                <button
                  type="submit"
                  className="mx-auto me-0 flex items-center"
                >
                  <p className="hover:underline-offset-2 hover:underline italic font-bold">
                    Next
                  </p>
          ...
          >
            {' '}
            {/* Chaflow Choosen Stage */}
            <div
              className="w-full h-[50%] bg-white rounded-md shadow-md shadow-yellow-300  hover:cursor-pointer"
              onClick={() => handleChatflowChoosen('AI')}
            >
            ...
            <div
              className="w-full h-[50%] bg-white rounded-md shadow-md shadow-yellow-300  hover:cursor-pointer"
              onClick={() => handleChatflowChoosen('ASSISTANT')}
            >
              <div className="flex shadow-inner shadow-slate-400 p-2 rounded-md">
                <div className="basis-1/4">
                  <Image
